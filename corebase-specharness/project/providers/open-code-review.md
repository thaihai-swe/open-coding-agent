...
`open-code-review` is an optional, opt-in review provider. CoreBase SpecHarness does not
install it, require it, or enable it by default. New installs keep
`providers.review.active: none` and `mode: optional` so `/harness-verify` can
close out without `ocr` on PATH.

Use this guide only after you decide this repository should run OCR.
...
After setup, set `providers.review.active: open-code-review` in
...
providers:
  review:
    ...
    mode: optional   # records missing/failed OCR as deferred
    # mode: required # blocks verify when ocr is missing or fails
```

Leave `mode: optional` while you are installing and authenticating OCR.
Switch to `mode: required` only when this project should fail closeout
without a successful `ocr review`.
...
- CoreBase SpecHarness does not reimplement OCR judgment.
...
- Security-sensitive path findings still escalate through CoreBase SpecHarness security policy checks.
