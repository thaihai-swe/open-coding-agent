...
def get_next_adr_number(root):
    adr_dir = Path(root) / "corebase-specharness/project/adr"
    max_num = 0
    if adr_dir.exists():
...
def adr_generate(args):
    ...
    relative_adr_path = f"corebase-specharness/project/adr/{num_str}-{title_slug}.md"
    ...
    if not getattr(args, "dry_run", False):
        ...
        adr_log_path = Path(root) / "corebase-specharness/memories/repo/adr-log.md"
        ...
        adr_index_path = Path(root) / "corebase-specharness/project/adr/index.md"
