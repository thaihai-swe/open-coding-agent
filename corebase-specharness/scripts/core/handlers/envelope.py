...
def _lifecycle(root):
    ...
    return Lifecycle(HarnessConfig(root / "corebase-specharness/project/harness-config.yaml"))
...
def skill_enter(args):
    ...
    next_action = ""
    if args.feature:
        next_action = f"python3 corebase-specharness/scripts/core/cli.py skill-exit --skill {args.skill} --feature {args.feature}"
...
def skill_exit(args):
    ...
    if handoff and not allowed and handoff not in shipped:
        raise ValueError(f"handoff {handoff} is not a shipped CoreBase SpecHarness skill")
