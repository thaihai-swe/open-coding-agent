...
Implementation safety guidance for code and scripts. Normative permission tiers and trust boundaries live in `corebase-specharness/memories/repo/core-policies.md` `## Security Policy`.
...
More rules → more freedom. Constraints increase agent autonomy by making incorrect paths fail fast. Agents run freely within checked boundaries. When an agent hits a boundary, the fix is to either widen the boundary (if the action was correct) or keep it (if the action was wrong). See CC-008 for the one-rule-per-mistake loop.
