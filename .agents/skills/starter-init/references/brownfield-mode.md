...
4. **Archaeology sweep check**: If the archaeology findings (baseline commands, security paths, and preserved behaviors) have already been recorded in `core-zero/memories/repo/project-knowledge-base.md` and `core-zero/memories/repo/core-policies.md` from a prior sweep, skip Phase A and continue to Phase B directly. If they are missing, run Phase A first.
