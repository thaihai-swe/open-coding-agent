...
| | |
|---|---|
| **Reads** | `manifest.json`, `core-zero/project/harness-config.yaml`, `core-zero/generated/gate-runs.json` (when present), `core-policies.md` |
| **Writes** | Optional: `core-zero/generated/harness-assessment.md`, `learned-heuristics.md` drafts, user-approved fixes |
| **Key CLI** | `python3 core-zero/scripts/core/cli.py doctor --json`, `python3 core-zero/scripts/core/cli.py gate-check --json`, `python3 core-zero/scripts/core/cli.py verify --feature <slug>` |
...
- **When to Use**:
  ...
  - Processing verification gate failures (`core-zero/generated/gate-runs.json`) to draft self-healing heuristics.
...
| `create` | Missing infrastructure detected | Scaffolds missing standard directories (`core-zero/generated/`, `memories/domain/`) |
| `improve` | Diagnostics failure analysis | Reviews failed entries in `core-zero/generated/gate-runs.json` and drafts `[DRAFT]` heuristics |
...
- **Reads**: `manifest.json`, `core-zero/project/harness-config.yaml`, `core-zero/generated/gate-runs.json` (when present), `core-zero/memories/repo/core-policies.md`.
- **Writes**:
  - `core-zero/generated/harness-assessment.md` (health report)
  - `core-zero/memories/repo/learned-heuristics.md` (appended `[DRAFT]` entries)
...
2. **Mode Execution Procedures**:
   - *Assess Mode*: Run `python3 core-zero/scripts/core/cli.py doctor --json`. Check `manifest.json` against actual file tree. Validate `core-zero/project/harness-config.yaml`. Detect orphan feature directories lacking `status.md`. Write `core-zero/generated/harness-assessment.md`.
   - *Create Mode*: Read assessment report; scaffold missing standard directories or baseline placeholders.
   - *Improve Mode*: Review failed entries in `core-zero/generated/gate-runs.json` when available. Draft evidence-backed `[DRAFT]` entries in `learned-heuristics.md`; do not promote them automatically.
   - *Eval Mode*: Audit `CC-*` identifiers in `core-policies.md` for sequentiality. Run `python3 core-zero/scripts/core/cli.py verify --feature <slug>` (or `python3 core-zero/scripts/core/cli.py doctor`). Compare manifest against file tree.
   - *Doctor Mode*: Run Assess steps, check for broken links in `skills/*/*.md`, and re-run `python3 core-zero/scripts/core/cli.py doctor --json`.
