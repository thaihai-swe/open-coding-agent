...
| **Writes** | Optional: `core-zero/memories/repo/`, `core-zero/memories/domain/`, `session-extracts.md` |
| **Key CLI** | `python3 core-zero/scripts/core/cli.py memory-audit --json`, `python3 core-zero/scripts/core/cli.py memory-gate --json`, `python3 core-zero/scripts/core/cli.py session-end --feature <slug>` |
...
| `audit-mode` | Invoked via `memory-audit` or when debugging memory budget | Reads `python3 core-zero/scripts/core/cli.py memory-audit --json`, reports line counts, token estimates, and threshold breaches |
| `compaction-mode` | File exceeds line/token threshold (via `python3 core-zero/scripts/core/cli.py memory-gate`) | Reduces prose by 30-50% while preserving every `##` heading and stable ID (`LH-*`, `CC-*`) |
| `decay-archival` | LH-* heuristics flagged as outdated or superseded | Moves tombstoned heuristics to `core-zero/memories/archive/deprecated-heuristics.md` |

## I/O & Artifact Protocol

- **Reads**: `artifacts/features/<slug>/session-extracts.md`, `core-zero/MASTER_INDEX.md`, `core-zero/memories/repo/core-policies.md`, `core-zero/memories/repo/learned-heuristics.md`, `core-zero/memories/repo/project-knowledge-base.md`.
- **Writes**:
  - `core-zero/memories/repo/learned-heuristics.md` (`LH-*`)
  - `core-zero/memories/repo/project-knowledge-base.md`
  - `core-zero/memories/repo/core-policies.md` (`CC-*`)
  - `core-zero/memories/domain/<name>/patterns.md`
  - `core-zero/memories/archive/deprecated-heuristics.md`
...
1. **Pre-flight & Mechanical Pre-flight**:
   - Run `python3 core-zero/scripts/core/cli.py memory-audit --json` to inspect file sizes, line counts, token estimates, and threshold warnings.
   - Run `python3 core-zero/scripts/core/cli.py memory-gate --json` to verify budget compliance.
...
2. **Mode Execution**:
   ...
   - *Compaction Mode*: Create safety snapshot (`<file>.bak` and `<file>.ids_before`). Reduce prose by 30–50% converting prose to bullets. Preserve every `##` heading and every stable ID (`LH-*`, `CC-*`, `INV-*`, `ADR-*`, `T-*`, `REQ-*`, `AC-*`). Validate `<file>.ids_after` matches `ids_before`.
   - *Decay Action*: Tombstone archived heuristics in `learned-heuristics.md` and append full body to `deprecated-heuristics.md`. Do not reuse retired IDs.

3. **Verification & Handoff**:
   - Re-run `python3 core-zero/scripts/core/cli.py memory-audit --json` to confirm size compliance.
   - Close feature session: Run `python3 core-zero/scripts/core/cli.py session-end --feature <slug>`.
