...
from core._lib.contracts import check_schema_version
...
class HarnessConfig:
    ...
    def _validate_lifecycle(lifecycle):
        ...
        if lifecycle.get("schema_version") != 1:
            raise ConfigError("state-machine.yaml schema_version must be 1")
    ...
    def validate(self):
        if not isinstance(self.data, dict):
            raise ConfigError("harness-config.yaml must be a mapping")
        failures = check_schema_version(
            self.path, "harness-config.yaml", "core-zero/project/harness-config.yaml"
        )
        if failures:
            raise ConfigError(failures[0])
        ...
        for key, value in thresholds.items():
            if key.endswith(("_lines", "_seconds", "_budget", "_warn", "_hard", "_breach")):
                try:
                    if int(value) <= 0:
                        raise ValueError
