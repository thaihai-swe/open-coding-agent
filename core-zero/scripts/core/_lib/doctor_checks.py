...
REQUIRED_SKILL_FIELDS = {"id", "name", "description", "tags", "triggers"}
COMMAND_RE = re.compile(r"(?:python3\s+)?(?:\./)?core-zero/scripts/core/cli\.py\s+([a-z](?:[a-z0-9-]*[a-z0-9])?)\b")


def check_contracts(root):
    from core._lib.contracts import (
        check_status_template_version,
        check_tool_providers_version,
        validate_manifest,
    )
    return (
        validate_manifest(root)
        + check_tool_providers_version(root)
        + check_status_template_version(root)
    )
...
def check_context_routes(root):
    ...
    state_machine = load_yaml(str(root / "core-zero/project/state-machine.yaml")) or {}
    machine_phases = {
...
def _skill_metadata(path):
    from core._lib.yaml_reader import loads

    text = path.read_text(encoding="utf-8", errors="ignore")
    if not text.startswith("---\n"):
        raise ValueError("missing frontmatter")
    _, frontmatter, _ = text.split("---", 2)
    metadata = loads(frontmatter) or {}
    if not isinstance(metadata, dict):
        raise ValueError("frontmatter must be a mapping")
    return text, metadata


def check_skill_contracts(root):
    ...
    for path in skill_files:
        ...
        try:
            text, metadata = _skill_metadata(path)
        except Exception as exc:
            failures.append(f"skill {name} has invalid frontmatter: {exc}")
            continue
        missing = REQUIRED_SKILL_FIELDS - metadata.keys()
        extra = metadata.keys() - REQUIRED_SKILL_FIELDS
        if missing:
            failures.append(f"skill {name} missing fields: {', '.join(sorted(missing))}")
        if extra:
            failures.append(f"skill {name} has unknown fields: {', '.join(sorted(extra))}")
        if metadata.get("name") != name:
            failures.append(f"skill {name} name must match its directory")
        if metadata.get("id") != f"skill-{name}":
            failures.append(f"skill {name} must use id skill-{name}")
        for field in ("description",):
            if not isinstance(metadata.get(field), str) or not metadata[field].strip():
                failures.append(f"skill {name} is missing {field}")
        for field in ("tags", "triggers"):
            if not isinstance(metadata.get(field), list) or not metadata[field]:
                failures.append(f"skill {name} must declare at least one {field[:-1]}")
        for command in COMMAND_RE.findall(text):
            if command.endswith("-"):
                continue
            if command not in COMMANDS:
                failures.append(f"skill {name} references unknown CLI command {command}")
        skills[name] = metadata

    routes = {row.get("skill") for row in load_routes(root).get("skills", [])}
    for name in sorted(skills.keys() - routes):
        failures.append(f"skill {name} has no context route")
    for name in sorted(routes - skills.keys()):
        failures.append(f"context route references missing skill {name}")
    return failures
...
def check_upgrade_contracts(root):
    ...
    machine = load_yaml(str(Path(root) / "core-zero/project/state-machine.yaml")) or {}
    transitions = {
...
def check_surface_integrity(root):
    root = Path(root)
    removed = [
        "core-zero/scripts/core/harness/cli.py", "core-zero/scripts/core/tool_providers.py",
        "core-zero/scripts/core/dashboard_generator.py", "core-zero/scripts/core/capability_recommender.py",
        "core-zero/scripts/core/catalog_generator.py", "core-zero/scripts/core/_lib/context_index.py",
        "core-zero/scripts/core/_lib/budget.py",
        "core-zero/scripts/core/_lib/telemetry_roi.py", "core-zero/scripts/core/_lib/telemetry_store.py",
        "core-zero/scripts/core/handlers/configuration.py", "core-zero/scripts/core/handlers/handoff.py",
        "core-zero/scripts/core/handlers/upgrades.py",
        "core-zero/scripts/core/readiness.py",
        ".corezero/engine", ".corezero/scripts",
    ]
    return [f"removed leftover remains: {path}" for path in removed if (root / path).exists()]
...
def check_static_audit(root):
    ...
    root = Path(root)
    core_root = root / "core-zero/scripts/core"
    paths = sorted(
...
def check_project_setup(root):
    ...
    config = HarnessConfig(Path(root) / "core-zero/project/harness-config.yaml")
    warnings = []
