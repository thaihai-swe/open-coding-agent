...
DEFAULT_RETRIEVAL_EXCLUDES = {
    ...
    "coverage", "__pycache__", "core-zero", "skills", "references", "artifacts",
}
...
def _context_settings(root):
    config = load_yaml(str(Path(root) / "core-zero/project/harness-config.yaml")) or {}
...
def _context_budget(root, profile, budget):
    ...
    configured = profile_config.get("payload") or context.get("payload_budget", 3000)
...
def _channel_limits(root):
    context = _context_settings(root)
    feature_limit = int(context.get("max_feature_tokens", 2000))
    project_limit = int(context.get("max_project_tokens", 1600))
    return {
        "bootstrap": int(context.get("max_bootstrap_tokens", 1200)),
        "project": project_limit,
        "feature": feature_limit,
        "task": feature_limit,
        "retrieved": int(context.get("max_retrieved_tokens", 2400)),
...
def _domain_packs(root, intent, settings):
    base = root / "core-zero/memories/domain"
...
def _channel_for_path(path, root):
    ...
    if relative.startswith("artifacts/") or relative.startswith(".corezero/"):
        return "feature"
    if relative.startswith("core-zero/memories/"):
        return "durable_memory"
    if relative.startswith("core-zero/rules/"):
        return "bootstrap"
    return "project"
...
def build_context_pack(
    ...
    if skill_name in {"spec-research", "spec-requirements", "spec-adr"}:
        policy_sections = ["Purpose", "Normative Rules", "Security Policy"]
    always = [
        ("core-zero/rules/caveman.md", "Must", None, "communication rule"),
        ("core-zero/memories/repo/core-policies.md", "Must", policy_sections, "runtime policy"),
    ]
    if skill_name in {"context-memory", "harness-maintain"}:
        always.append(("core-zero/rules/headroom.md", "Must", ["Rules"], "context rule"))
    ...
    for candidate in candidates:
        ...
        selected_item = {
            ...
            "trust": item.get("trust", "trusted_kit" if relative.startswith("core-zero/") else "repository_evidence"),
    ...
    if delta_from:
        try:
            ...
            pack["delta"] = True
            pack["delta_from"] = str(delta_from)
