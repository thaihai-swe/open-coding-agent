...
from core._lib.contracts import check_tool_providers_version
...
def _provider_config(root):
    path = Path(root) / "core-zero/project/tool-providers.md"
    if not path.is_file():
        return {}
...
def validate_provider_contract(root):
    errors = check_tool_providers_version(root)
    try:
