...
def _thresholds(root: Path) -> dict[str, int]:
    config = load_yaml(str(root / "core-zero/project/harness-config.yaml")) or {}
...
def _memory_files(root: Path, thresholds: dict[str, int]) -> list[dict[str, Any]]:
    ...
    domain_base = root / "core-zero/memories/domain"
...
def _semantic_memory_summary(root: Path) -> dict[str, Any]:
    ...
    lh_path = root / "core-zero/memories/repo/learned-heuristics.md"
    adr_path = root / "core-zero/memories/repo/adr-log.md"
    ...
    if adr_path.is_file():
        ...
        adr_count = len([line for line in text.splitlines() if line.startswith("### ADR-")])
    return {
