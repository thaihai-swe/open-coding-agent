...
def init(args):
    ...
    config_path = root / "core-zero/project/harness-config.yaml"
    try:
...
def _valid_handoffs(root, feature):
    machine = Lifecycle(HarnessConfig(root / "core-zero/project/harness-config.yaml"))
...
def status(args):
    ...
    next_action = features[0].get("next_step", "") if len(features) == 1 else "python3 core-zero/scripts/core/cli.py status"
    ...
    return _result("status", feature=args.feature, warnings=[f"Blocker: {slug}" for slug in blockers],
...
def verify(args):
    ...
    if not args.dry_run:
        _append_generated_record(
            root / "core-zero/generated/gate-runs.json",
            {"feature": args.feature, "gates": gate_results},
        )
        _append_generated_record(
            root / "core-zero/generated/provider-runs.json",
            {
    ...
    if not args.dry_run:
        ...
        _append_generated_record(root / "core-zero/generated/verification-runs.json", record)
...
def evaluate_artifact_check(root, feature, skill="", phase="", trace=False):
    ...
    enforcement_mode = HarnessConfig(root / "core-zero/project/harness-config.yaml").verification_mode()
    return _result(
        "artifact-check",
